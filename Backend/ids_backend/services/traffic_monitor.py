    ...
    def _process_packets(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                anomalies = self.packet_analyzer.check_packet_anomalies(packet_info)
                for anomaly in anomalies:
                    alerts.append(Alert(
                        alert_type=f"PACKET_ANOMALY:{anomaly.split(':')[0]}",
                        source_ip=packet_info.source_ip,
                        destination_ip=packet_info.destination_ip,
                        destination_port=packet_info.destination_port,
                        description=anomaly,
                        level=AlertLevel.MEDIUM,
                        metadata={'packet_size': packet_info.size}
                    ))
                ...
                if self.anomaly_detector:
                    anomaly_alerts = self.anomaly_detector.analyze(packet_info)
